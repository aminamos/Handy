...
async getHistoryUsageStats() : Promise<Result<HistoryUsageStats, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("get_history_usage_stats") };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
export type HistoryUsageStats = { today: string; total_words: number; today_words: number; this_week_words: number; daily_word_counts: DailyWordCount[]; weekly_word_counts: WeeklyWordCount[] }
/**
...
export type WeeklyWordCount = { week_start: string; words: number }
